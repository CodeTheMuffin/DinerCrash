...
public class OrderBox : MonoBehaviour
{
    ...
    public void setOrderForm(OrderForm form)
    {
        ...
        { optionVisibility((int)OrderOptions.Options.warm_milk, true); }
    }
    ...
    public void reassignAllOrderOptions()
    {
        ...
        {
            if (OrderOptions.option_names_to_index.ContainsKey(child.name))
            {
                int index = OrderOptions.option_names_to_index[child.name];

                // If the child's name exists as a value in the Options enum
                if (System.Enum.IsDefined(typeof(OrderOptions.Options), index))
                {
